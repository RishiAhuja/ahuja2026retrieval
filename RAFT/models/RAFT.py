...
class Model(nn.Module):
    ...
    def __init__(self, configs, individual=False):
        ...
        # Use MPS for Mac, CUDA for GPU, or CPU as fallback
...
#                 configs.enc_in * configs.seq_len, configs.num_class)
    ...
    def encoder(self, x, index, mode):
        ...
        # Fix: Index on CPU then move to device to avoid device mismatch
        # retrieval_dict is stored on CPU after empty_cache() in prepare_dataset
        index_cpu = index.cpu() if index.is_cuda else index
        
        # Fix: Clamp indices to valid range to prevent off-by-one errors
        # When dataset size doesn't divide evenly by batch size, last batch may have out-of-range indices
