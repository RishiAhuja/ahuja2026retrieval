...
def time_warp(x, sigma=0.2, knot=4):
    ...
    for i, pat in enumerate(x):
        for dim in range(x.shape[2]):
            time_warp_val = CubicSpline(warp_steps[:, dim], warp_steps[:, dim] * random_warps[i, :, dim])(orig_steps)
            scale = (x.shape[1] - 1) / time_warp_val[-1]
            ret[i, :, dim] = np.interp(orig_steps, np.clip(scale * time_warp_val, 0, x.shape[1] - 1), pat[:, dim]).T
    return ret
...
def spawner(x, labels, sigma=0.05, verbose=0):
    ...
    ret = np.zeros_like(x)
    for i, pat in enumerate(x):
        # Ensure same sample isn't selected
    ...
    for i, pat in enumerate(x):
        ...
        if choices.size > 0:
            ...
            if dtw_type == "shape":
                ...
                path = dtw.dtw(random_prototype, pat, dtw.RETURN_PATH, slope_constraint=slope_constraint,
                               window=window)
    ...
    if use_variable_slice:
        ...
        if max_warp == 0:
            ...
            for i, pat in enumerate(ret):
                # Variable slicing based on warp magnitude
...
def run_augmentation(x, y, args):
    """Run data augmentation on full dataset."""
...
def run_augmentation_single(x, y, args):
    """Run data augmentation on a single series or batch."""
    np.random.seed(args.seed)
    ...
    if len(x.shape) < 3:
        # Whole-series augmentation: (sequence_length, num_channels) -> (1, sequence_length, num_channels)
        x_input = x[np.newaxis, :]
    elif len(x.shape) == 3:
        # Batch augmentation: keep (batch_size, sequence_length, num_channels)
        x_input = x
    else:
...
def augment(x, y, args):
    """Apply selected augmentation transforms."""
